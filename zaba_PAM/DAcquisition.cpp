...
//*****************************************************************************
  ...
  *fLog << "\t\t\t" << " ----- constructing modules:" << endl;
  ...
  fModuleList->Add(fDCAEN1740D  = new DCAEN1740D((char*)"CAEN 1740D Master",     0x34100000));
  fModuleList->Add(fDCAEN1740D  = new DCAEN1740D((char*)"CAEN 1740D Slave",      0x33100000));
  //fModuleList->Add(fDCAEN1740D  = new DCAEN1740D((char*)"CAEN 1740D",      0x32100000));

  //int nrDigitiser = 1;
  //UInt_t AddrDigitiser[0x34100000, 0x33100000];

  /*     for (int i=0; i < nrDigitiser; i++){
         char nomeDigitiser [50];
         sprintf(nomeDigitiser, "CAEN_1740D_DigNo%d(%03d)", i+1);
         fModuleList->Add(fDCAEN1740D  = new DCAEN1740D(*nomeDigitiser, AddrDigitiser[i]));
     }
*/
...
bool DAcquisition::CheckCondition(){
  ...
  if(m_StopAfterEventsEntry){
	 ...
	 m_StopCondition = 3;
         cout << "CheckCondition FALSE: " << m_StopCondition << endl;
         return kFALSE;
       }
     }
