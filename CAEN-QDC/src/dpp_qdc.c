...
/*
	...
	uint32_t k = 0;
        for(j=0; j<NumEvents[k] && k < 8; j++, ++k) {
            printf("----- event j=%d k=%d\n", j, k);		    
	    for (i2 = 0; i2 < 8; i2++) {
	    printf("Gr.%5d %5d\t%5d\t%5d\t%5d\t%5d\t%5d\t%5d\t%5d\n", i2, 
        ...
        for (i=0; i < gEquippedChannels; ++i) {              
  		...
  		gPrevTimeTag[i]   = gEvent[i][j].TimeTag;
            ...
            if (gToggleTrace) {
                ...
                switch (gAnalogTrace) {
                case 0:
                    CAEN_DGTZ_WriteRegister(gHandle, 0x8008, 3<<12); // clear bit in register 0x8000
                    break;
                case 1:
                    CAEN_DGTZ_WriteRegister(gHandle, 0x8004, 1<<12); // set bit
                    CAEN_DGTZ_WriteRegister(gHandle, 0x8008, 1<<13); // clear bit
                    break;
                case 2:
...
int cleanup_on_exit() {
	...
	/* deallocate memory for gro{up events */
	for(i=0; i<8; i++) {  
    ...
    if(ret != CAEN_DGTZ_Success )
       printf("error in cleanup_on_exit %d\n", ret);	    
	return 0;
    ...
    */
    ...
    if (!gAcquisitionBufferAllocated) {
	  ...
	  */
      ...
      if(gHisto == NULL) {printf("Cannot allocate %ld bytes for histogram! Exiting...", gEquippedChannels * sizeof(uint32_t *));exit(-1);}
      for(i= 0 ; i < gEquippedChannels; ++i) {
	    gHisto[i] = (uint32_t *)malloc(HISTO_NBIN * sizeof(uint32_t));
        if(gHisto[i] == NULL) {printf("Cannot allocate %ld bytes for histogram! Exiting...", HISTO_NBIN * sizeof(uint32_t));exit(-1);}
      }

	  /* open gnuplot in a pipe and the data file*/
      gHistPlotFile = popen("/usr/bin/gnuplot", "w");
    ...
    for(i=0; i < gEquippedChannels; ++i) {
          char filename[200];
          sprintf(filename, "list_ch%02d.txt", i);
