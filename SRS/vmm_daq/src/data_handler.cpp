...
DataHandler::DataHandler(QObject *parent) :
    ...
    //test shared
    // remove monitoring m_daqConf(0),
    // remove monitoring m_ce(0),
    // remove monitoring m_sh(0),
    //thread
    m_DAQSocket(0), 
    m_mapping_file(""),
    m_daqSocket(0),
    ...
    m_eventCountStop(-1),
    //daqmon
    // remove monitoring m_daqMonitor(0),
    m_fileOK(false),
    m_outDir(""),
    ...
    m_artTree(NULL)
{
    ...
    // remove monitoring m_daqMonitor = new DaqMonitor();
    // remove monitoring connect(m_daqMonitor, SIGNAL(daqHangObserved()), this, SLOT(daqHanging()), Qt::QueuedConnection);
...
{
    m_dbg = doit;
    // remove monitoring m_daqMonitor->setDebug(doit);
}
// ------------------------------------------------------------------------ //
...
void DataHandler::startDAQMonitor()
{
    ...
    // remove monitoring m_daqMonitor->setTimer();
    // remove monitoring m_daqMonitor->setCounter(n_daqCnt);
    // remove monitoring m_daqMonitor->startTimer();
}
// ------------------------------------------------------------------------ //
//daqmon
void DataHandler::daqHanging()
{
    ...
    closeDAQSocket();
    // wait a second    
    // remove monitoring  boost::this_thread::sleep(boost::posix_time::seconds(1));
    usleep(1000000);
    connectDAQSocket();
}
// ------------------------------------------------------------------------ //
