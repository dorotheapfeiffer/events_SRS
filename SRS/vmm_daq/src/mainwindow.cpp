    ...
    daq_act (DAQS_PER_GUIWINDOW)
{
    ...
    watcher->addPath("/home/michael/Readout_Software/configs/");
    connect(watcher, SIGNAL(directoryChanged(QString)), this, SLOT(slotFileChanged(QString)));
    connect(watcher, SIGNAL(fileChanged(QString)), this, SLOT(slotFileChanged2(QString)));
    lastModified = "";
}
...
MainWindow::~MainWindow()
{
    ...
    //f1 = QtConcurrent::run(b, &Commandline::EndCommandline);
    f1.waitForFinished();
...
// ------------------------------------------------------------------------- //
void MainWindow::slotFileChanged(QString directory) // monitor the config path and add files which are changed to the list of monitored files
// this slot is called often, even when the file is touched by in the text editor and 3x when it is saved
// adding the file that is changed to the path and then monitoring this file is only executed once, when the file is saved (slotFileChanged2)
{
    cout << "A file in directory " << directory.toStdString() << " changed." << endl;
    QDir dir;
    dir.setPath(directory);
    QFileInfoList list = dir.entryInfoList(QDir::NoDotAndDotDot | QDir::Files, QDir::Time);

    QString name = list.first().baseName();

    QString addName = "/home/michael/Readout_Software/configs/";
    addName += name;
    addName += ".txt";
    watcher->addPath(addName);
}
void MainWindow::slotFileChanged2(QString file) // monitors the file that has been added by slotFileChanged
{
    cout << " Config file " << file.toStdString() << " changed." << endl;
    //watcher->addPath(file);
    const char *chr = file.toStdString().c_str();
    //vmmconfhandl->LoadSingleVMMConf(chr);
    //configHandle().LoadConfig(file);
    //updateConfigState();
}
