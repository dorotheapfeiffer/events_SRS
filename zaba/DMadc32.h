...
*/
 ...
 address = m_BaseAddress + 0x6042;
 //printf("CAENVME_WriteCycle: Set ADC resolution to %d\n", m_ADCResolution);
 ret = CheckError( CAENVME_WriteCycle(m_VMEBridge, address, &m_ADCRange, cvA32_U_DATA, cvD16) );
 if(ret) return;
...
//-----------------------------------------------------------------------------
 void DMadc32::Log(std::ofstream & logfile) {
  ...
  logfile << "\t\t\t     + m_ADCRange     : " << m_ADCRange        << std::endl;
...
void DMadc32::ShowSettings() {
  ...
  std::cout << "\tm_ADCRange:      " << m_ADCRange        << std::endl; 
  std::cout << "=====================================\n"<< std::endl; 
}
...
//==================================================================================
 ...
 fout << std::endl;
 fout << "#Set the ADC Range"                  << std::endl; 
 fout << "ADCRange "      	<< m_ADCRange   << std::endl; 
 fout << std::endl;
   ...
   while (getline(inpfile,line)) {
    ...
    value = Trim(line.substr(posEqual+1));
     ...
     if( inSection == string("Mesytec Madc-32")){
       ...
       else if( name == string("ADCRange") ){
         temp = atoi( value.c_str() );
         if(temp < 0 || temp > 5) m_ADCRange = 2;
         else m_ADCRange = temp;
         }
