...
void MainWindow::AnalyseData(){
   ...
   cout << "DEBUG [MainWindow::AnalyseData] iNrFile: "<< iNrFile << "\tnr of files:" <<  Files::getInstance().GetSize() << endl;
   Int_t i = 0;
   ...
   cout << "DEBUG [MainWindow::AnalyseData] exit " << endl;
} 
//============================================================
...
void MainWindow::DoAlgorythm() {
   ...
   if(mAlgorythm == 0) {
      ...
      mAlgorythm = 1;
      WriteConfigFile();
      if(mUserClass) mUserClass->DrawCanvas( mAlgorythm );
      }
   else{ 
