...
//*****************************************************************************
 DCAEN1740::DCAEN1740(Char_t *mdesc, UInt_t addr) : 
  ...
  //clear the buffer
  for(Int_t i = 0; i < MAX_BLT_SIZE; i++)
     m_Buffer[i] = 0;
...
//-----------------------------------------------------------------------------
 void DCAEN1740::ShowData(DGDisplay *fDisplay, DMultiGrid *fMultiGrid) {
  ...
  uint32_t *ptr = (uint32_t*)m_localBuffer;
  for(Int_t i = 0; i < m_dataSizeByte/4; i++){
      m_Header = ptr[i];
      ...
      m_Footer = ptr[i+n+1]; // here check! if this is OK
...
//-----------------------------------------------------------------------------
void DCAEN1740::DataSave(DMultiGrid *fMultiGrid){
   ...
   //std::cout << "[DEBUG] 1 DCAEN1740::DataSave m_EmptyBuffer = "<< fMultiGrid->m_EmptyBuffer << std::endl;
   if( !VME_CRATE ) m_IRQ = 1; 
   if( fMultiGrid->m_EmptyBuffer ) m_IRQ = 1; 
   //std::cout << "[DEBUG] 2 DCAEN1740::DataSave m_EmptyBuffer = "<< fMultiGrid->m_EmptyBuffer << std::endl;
   if(!m_IRQ) return;
      m_IRQ = 0;
   //std::cout << "[DEBUG] 3 DCAEN1740::DataSave m_EmptyBuffer = "<< fMultiGrid->m_EmptyBuffer << std::endl;
  ...
  if( m_saveAfterSize || m_saveAfterTime || fMultiGrid->m_EmptyBuffer){
    std::cout << "[DEBUG] 4 DCAEN1740::DataSave m_EmptyBuffer = "<< fMultiGrid->m_EmptyBuffer << std::endl;
    char nr[256];
...
void DCAEN1740::ShowSettings() {
  std::cout << "[MESSAGE] ======= DCAEN1740::ShowSettings =======" << std::endl; 
